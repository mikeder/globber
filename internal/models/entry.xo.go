...
type Entry struct {
	ID        int           `db:"id"`        // id
	AuthorID  int           `db:"author_id"` // author_id
	Slug      string        `db:"slug"`      // slug
	Title     string        `db:"title"`     // title
	Markdown  string        `db:"markdown"`  // markdown
	HTML      string        `db:"html"`      // html
	Published time.Time     `db:"published"` // published
	Updated   time.Time     `db:"updated"`   // updated
	Highlight sql.NullInt64 `db:"highlight"` // highlight
...
func BulkInsertEntrys(ctx context.Context, db XODB, es []*Entry) error {
	for i := 0; i < len(es); i += chunkSize {
		end := i + chunkSize
		if end > len(es) {
			end = len(es)
		}
		...
		// sql insert query, primary key provided by autoincrement
		sqlstr := "INSERT INTO blog.entries (author_id, slug, title, markdown, html, published, updated, highlight)" +
			"VALUES " +
			placeholders

		var args []interface{}
		for _, e := range chunk {
			args = append(args, e.AuthorID, e.Slug, e.Title, e.Markdown, e.HTML, e.Published, e.Updated, e.Highlight)
		}
